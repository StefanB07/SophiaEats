...
public class InMemoryDeliveryCatalogRepository implements DeliveryCatalogRepository {
    ...
    public boolean consumeCapacity(String restaurantId, String label) {
        ...
        while (it.hasNext()) {
            ...
            if (s.getLabel().equalsIgnoreCase(label)) {
                ...
                if (newCap <= 0) {
                    it.remove();           // cÃ¢nd ajunge la 0, dispare slotul
                } else {
                    s.setCapacity(newCap);  // altfel doar decrementÄƒm
                }
                return true;
            }
        }
        return false; // nu am gÄƒsit slotul
    }
